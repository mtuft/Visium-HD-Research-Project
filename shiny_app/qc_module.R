...
# UI fragment (PCA/clustering tab sidebar): PC-count slider, clustering-resolution
# slider, and the clustering-method selector (Sketch [leverage-score] vs BANKSY
# [spatially-aware]), with the "Run UMAP & Clustering" action.
pcaUI <- function() {
  # [REDACTED — UI layout]
}

# Input validation: rejects non-Seurat input, empty objects, and objects that
# already contain reductions or clustering (the app requires raw, unprocessed input).
...
# Server: QC + PCA + clustering control logic. Handles the live dataset summary;
# distribution-aware QC threshold guidance (MAD vs percentile reasoning for 8um
# bins); interactive sliders with +/- steppers; live retained-spot count with
# safety thresholds; BPCells on-disk status; and submission of Step 1 (filter +
# normalise + PCA) and Step 2 (UMAP + clustering) jobs to the background worker
# via the file-based queue.
...
# QC violin plots (counts, features, % mitochondrial) with threshold overlays;
# downsamples to 50k spots for rendering performance.
qcPlotServer <- function(output, input, raw_obj) {
  # [REDACTED — plot construction]
}

# Elbow plot: % variance explained per PC with selected-PC highlight and
# cumulative-variance annotation; includes a PDF download handler.
elbowPlotServer <- function(output, input, pca_obj) {
  # [REDACTED — plot construction]
}

# Variable-features plot: mean-variance scatter highlighting highly variable genes
# with top-10 gene labels; includes a PDF download handler.
